...
struct test
{
  static void run()
  {
    ...
    unsigned int number_of_it = 100000;
    ...
    while (index < number_of_it)
      {
        ...
        if (Template_Test==BMSH_RNEA)
          metapod::rnea<Robot,true>::run(robot,Q, QDot, QDDot);
        else if (Template_Test==BMSH_CRBA)
          metapod::crba<Robot,true>::run(robot,Q);
